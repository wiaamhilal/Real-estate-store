...
import {clearBasket} from "../redux/UserSlice";

const Payment = () => {
  const {basket, user, location} = useSelector((state) => state.UserSlice);
      ...
      <Main className="container text-color ">
        ...
        <div className="d-block d-sm-flex align-items-center delevery-box">
          ...
          <div style={{flex: "1"}}>
            ...
            <button className="btn btn-primary w-100 mb-1" onClick={buyNow}>
              Buy now
            </button>
          </div>
        </div>
